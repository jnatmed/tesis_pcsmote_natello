...
class PCSMOTE(Utils):
    ...
    def _generar_sinteticas_binario(
        ...
        # si no hay candidatas, no puedo generar nada
        if len(indices_locales_candidatas) == 0:
            return None
        
        factor_cap = 1.25  # o 1.5

        n_candidatas = int(len(indices_locales_candidatas))

        cap_base = int(np.ceil(cantidad_sinteticas_objetivo / n_candidatas))
        cap_por_semilla = max(
            1,
            int(np.ceil(factor_cap * cap_base))
        )

        # Garantía: que haya capacidad suficiente para llegar a G
        cap_por_semilla = max(
            cap_por_semilla,
            cap_base
        )

        cantidad_generadas = 0
        intentos = 0
        max_intentos = int(cantidad_sinteticas_objetivo * 20)  # corte defensivo

        while cantidad_generadas < cantidad_sinteticas_objetivo and intentos < max_intentos:
            intentos += 1

            # semillas que todavía no alcanzaron el cap
            semillas_disponibles = []
            for i in indices_locales_candidatas:
                if conteo_sinteticas_por_semilla[int(i)] < cap_por_semilla:
                    semillas_disponibles.append(int(i))

            if len(semillas_disponibles) == 0:
                if hasattr(self, "logger") and self.logger is not None:
                    self.logger.info(
                        f"PCSMOTE: clase {clase_objetivo} sin semillas disponibles por cap "
                        f"(generadas={cantidad_generadas}/{cantidad_sinteticas_objetivo}, "
                        f"cap={cap_por_semilla}, candidatas={n_candidatas}, intentos={intentos})"
                    )
                break

            indice_local_semilla = int(
                rng.choice(semillas_disponibles)
            )
            ...
            cantidad_generadas += 1

        if cantidad_generadas < cantidad_sinteticas_objetivo:
            if hasattr(self, "logger") and self.logger is not None:
                self.logger.info(
                    f"PCSMOTE: clase {clase_objetivo} generó {cantidad_generadas}/{cantidad_sinteticas_objetivo} "
                    f"(cap={cap_por_semilla}, candidatas={len(indices_locales_candidatas)}, intentos={intentos})"
                )
